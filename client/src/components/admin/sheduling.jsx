...
export default function sheduling() {
    ...
    const scheduleRequest = allRequestVendor.filter((item)=>{
        return item.schedule !== null && item.status !== "cancelled";
    })
    ...
    return (
        <div className="relative">
            ...
            </h1>
            {
                ...
                {
                    scheduleRequest && scheduleRequest.length > 0 ?
                    scheduleRequest.map((singleRequest, index) => <RequestCard singleRequest={singleRequest} handleSchedle={handleSchedle} key={index} />)
                    : <h1 className="text-gray-600 text-lg">Empty Scheduled list.....</h1>
                }
                </div>: <LoadingPage/>
            }
        </div>
